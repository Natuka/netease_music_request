...
enum CRYPTO { WEAPI, LINUX_API, EAPI }
...
abstract class Request {
  ...
  static Future<PersistCookieJar> get _cookieJar async {
    if (cookieJar == null) {
      ...
      if (cookieDir == null) {
        cookieDir = '${appDocPath}${Platform.pathSeparator}cookies';
      }
  ...
  Future<Response> request(METHOD method, String url,
      Map<dynamic, dynamic> data, Map<dynamic, dynamic> options) async {
    await _cookieJar;
    // headers
    var headers = {
      'User-Agent':
          getUserAgent(options.containsKey('ua') ? options['ua'] : null)
    };
    ...
    if (options.containsKey('cookie') &&
        options['cookie'] is Map<String, dynamic>) {
    ...
    if (options.containsKey('crypto')) {
      if (options['crypto'] == CRYPTO.WEAPI) {
        ...
        header = {
          'osver':
              cookieMap.containsKey('osver') ? cookieMap['osver'] : '', //系统版本
          'deviceId': cookieMap.containsKey('deviceId')
              ? cookieMap['deviceId']
              : '', //encrypt.base64.encode(imei + '\t02:00:00:00:00:00\t5106025eb79a5247\t70ffbaac7')
          'appver': cookieMap.containsKey('appver')
              ? cookieMap['appver']
              : '6.1.1', // app版本
          'versioncode': cookieMap.containsKey('versioncode')
              ? cookieMap['versioncode']
              : '140', //版本号
          'mobilename': cookieMap.containsKey('mobilename')
              ? cookieMap['mobilename']
              : '', //设备model
          'buildver': cookieMap.containsKey('buildver')
              ? cookieMap['buildver']
              : DateTime.now().toString().substring(0, 10),
          'resolution': cookieMap.containsKey('resolution')
              ? cookieMap['resolution']
              : '1920x1080', //设备分辨率
          '__csrf': csrfToken,
          'os': cookieMap.containsKey('os') ? cookieMap['os'] : 'android',
          'channel':
              cookieMap.containsKey('channel') ? cookieMap['channel'] : '',
          'requestId':
              '${DateTime.now()}_${Random().nextInt(1000).toString().padLeft(4, '0')}'
        };
    ...
    return dio
        .request(url, data: data, options: requestOptions)
        .then((response) {
  ...
  Map<String, String> weapiEncrypt(Object data) {
    ...
    return {
      'params': aesEncrypt(
          aesEncrypt(json, ModeAES.cbc, BASE64_PRESET_KEY, BASE64_IV),
          ModeAES.cbc,
          base64.encode(secretKey),
          BASE64_IV),
      'encSecKey': hex.encode(
          base64.decode(rsaEncrypt(utf8.decode(reversedKey), RSA_PUBLIC_KEY)))
    };
  }
    ...
    return {
      'eparams': hex
          .encode(base64.decode(
              aesEncrypt(base64Json, ModeAES.ecb, BASE64_LINUX_API_KEY, '')))
          .toUpperCase()
    };
  }
  ...
  Map<String, String> eapiEncrypt(String url, Object data) {
    ...
    var digest =
        hex.encode(base64.decode(HashCrypt(ModeHash.MD5).hash(message)));
    var res = '${url}-36cd479b6b5-${json}-36cd479b6b5-${digest}';
    var base64Res = base64.encode(utf8.encode(res));
    return {
      'params': hex
          .encode(base64
              .decode(aesEncrypt(base64Res, ModeAES.ecb, BASE64_EAPI_KEY, '')))
          .toUpperCase()
    };
  }

  String eapiDecrypt(String data) =>
      aesDecrypt(data, ModeAES.ecb, BASE64_EAPI_KEY, '');

  List<int> getSecretKey() =>
      List<int>.generate(16, (i) => Random().nextInt(256))
          .map((e) => BASE62.codeUnitAt(e % 62))
          .toList();
}
